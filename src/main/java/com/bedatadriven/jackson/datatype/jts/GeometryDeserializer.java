...
public class GeometryDeserializer extends JsonDeserializer<Geometry> {
	...
	private GeometryCollection parseGeometryCollection(JsonNode root)
			throws JsonMappingException {
		return gf
				.createGeometryCollection(parseGeometries(root.get(GEOMETRIES)));
	}
	...
	private MultiLineString parseMultiLineStrings(JsonNode root) {
		return gf
				.createMultiLineString(parseLineStrings(root.get(COORDINATES)));
	}
	...
	private Polygon parsePolygonCoordinates(JsonNode arrayOfRings) {
		return gf.createPolygon(parseLinearRing(arrayOfRings.get(0)),
